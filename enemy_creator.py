...
def enemy_creator(game: Game, enemy_group: pg.sprite.Group, phase: int, wave: int, multiplicand: int) -> None:
    ...
    for i in range(len(stgs.enemy_waves[phase][wave])):
        if stgs.enemy_waves[phase][wave][i] == 1:
            EnemyShip1(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
        elif stgs.enemy_waves[phase][wave][i] == 2:
            EnemyShip2(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
        elif stgs.enemy_waves[phase][wave][i] == 3:
            EnemyShip3(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
        elif stgs.enemy_waves[phase][wave][i] == 100:
            Boss1(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
        elif stgs.enemy_waves[phase][wave][i] == 120:
            Boss2(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
        elif stgs.enemy_waves[phase][wave][i] == 140:
            Boss3(game, stgs.enemy_waves[phase][wave][i], enemy_group, (x_pos + x_pos * i, -50), phase, multiplicand)
